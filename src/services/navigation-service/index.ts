...
app.get('/mapa/data', async (req: Request, res: Response) => {
  try {
...
app.post('/google/compute-route', authenticate, async (req: Request, res: Response) => {
  try {
    const { originCoords, destinationCoords, routingPreference = 'TRAFFIC_UNAWARE' } = req.body;
    if (!originCoords || !destinationCoords) return res.status(400).json({ error: 'Faltan coordenadas' });

    const apiKey = process.env.GOOGLE_ROUTES_API_KEY;
    const response = await fetch('https://routes.googleapis.com/directions/v2:computeRoutes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Goog-Api-Key': apiKey as string, 'X-Goog-FieldMask': 'routes.duration,routes.distanceMeters,routes.polyline.encodedPolyline' },
      body: JSON.stringify({
