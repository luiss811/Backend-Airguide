 ...
 */
export async function evaluarEvento(nuevoEvento: any): Promise<any> {
    ...
    for (const eventoAfectado of eventosConflicto) {
      ...
      if (prioridadNuevo < prioridadAfectado) {
        ...
        if (eventoAfectado.creador?.correo) {
          await enviarNotificacionDesplazamiento(
            eventoAfectado.creador.correo,
            eventoAfectado.creador.nombre,
            eventoAfectado.nombre,
            eventoAfectado.edificio.nombre,
            edificioNuevo?.nombre || 'Nuevo Edificio',
            eventoAfectado.fecha_inicio
          );
        }
