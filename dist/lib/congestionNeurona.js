...
export async function entrenarCongestion() {
    if (isTraining)
        throw new Error("El modelo ya se está entrenando en este momento.");
    isTraining = true;
    try {
...
export async function predecirCongestionRuta(idRuta, horaLocal) {
    ...
    const score = modeloRef.predict(inputData);
    const arrayScore = await score.data();
    inputData.dispose();
    score.dispose();
    return arrayScore[0]; // Retorna entre 0 y 1
}
//# sourceMappingURL=congestionNeurona.js.map