...
app.post('/entrenar-neurona', authenticate, requireAdmin, async (req, res) => {
    try {
...
app.post('/:id(\\d+)/confirmar-asistencia', async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/:id(\\d+)/invitados', authenticate, async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/:id(\\d+)/registrar-invitado', async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/confirmar-ticket/:id_invitado(\\d+)', async (req, res) => {
    try {
